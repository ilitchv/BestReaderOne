...
export const AdminDashboard: React.FC<AdminProps> = ({ settings, setSettings, close }) => {
    ...
    const [activeTab, setActiveTab] = useState<'calibration' | 'sniper'>('calibration'); // NEW TAB STATE
    ...
    return (
        <div className="flex flex-col h-full bg-slate-900 animate-in slide-in-from-right duration-300 w-full rounded-xl overflow-hidden text-white">
            <div className="p-4 md:px-8 md:py-6 flex items-center justify-between border-b border-white/10 bg-slate-800">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === 'calibration' ? 'bg-cyan-500 text-black' : 'bg-slate-700 text-slate-400 hover:text-white'}`}
                    >
                        Calibration
                    </button>
                    <button
                        onClick={() => setActiveTab('sniper')}
                        className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors flex items-center gap-2 ${activeTab === 'sniper' ? 'bg-red-600 text-white' : 'bg-slate-700 text-slate-400 hover:text-white'}`}
                    >
                        <span>🎯</span> Sniper Engine
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 relative">
                ...
                {activeTab === 'sniper' && (
                    <div className="w-full h-full rounded-xl overflow-hidden border border-slate-700 shadow-2xl relative">
                        <iframe
                            src="/sniper-live/index.html"
                            className="w-full h-full border-0 bg-slate-900"
                            title="Sniper Engine Live"
                        />
                        <div className="absolute top-2 right-4 pointer-events-none bg-black/50 px-2 py-1 rounded text-[10px] text-white/50">
                            v7.1 Integration Protocol
                        </div>
                    </div>
                )}

                {/* CALIBRATION TAB CONTENT (Existing) */}
