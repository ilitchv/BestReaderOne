...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose, onRelocate }) => {
    ...
    useEffect(() => {
        const handleScanResult = (e: CustomEvent) => {
            setSearchTerm(e.detail.code);
            setSalesViewMode('tickets');
        };

        const handleSwitchToRisk = () => {
            setActiveTab('sales');
            setSalesViewMode('risk');
        };

        window.addEventListener('SCAN_RESULT', handleScanResult as any);
        window.addEventListener('SWITCH_TAB_TO_RISK_MONITOR', handleSwitchToRisk as any);

        return () => {
            window.removeEventListener('SCAN_RESULT', handleScanResult as any);
            window.removeEventListener('SWITCH_TAB_TO_RISK_MONITOR', handleSwitchToRisk as any);
        };
    }, []);

    // New: Check for redirection flag from Playground
    useEffect(() => {
        if (localStorage.getItem('BEAST_REDIRECT_TO_RISK') === 'true') {
            setActiveTab('sales');
            setSalesViewMode('risk');
            localStorage.removeItem('BEAST_REDIRECT_TO_RISK');
        }
    }, []);
    ...
    useEffect(() => {
        ...
        const getLocalISODateStr = (dateStr: string) => {
        ...
        if (dateRange.start && dateRange.end) {
            res = res.filter(t => {
                const tDate = getLocalISODateStr(t.transactionDateTime);
                return tDate >= dateRange.start && tDate <= dateRange.end;
            });
        } else if (dateRange.start) {
            res = res.filter(t => getLocalISODateStr(t.transactionDateTime) === dateRange.start);
        }
    ...
    const handleSaveUser = async (e: React.FormEvent) => {
        ...
        const userData: User = {
            ...
            referralCode: editingUser?.referralCode || `REF-${userId.substring(0, 5).toUpperCase()}`,
            levels: editingUser?.levels || { direct: 0, indirect: 0, deep: 0 },
            isVoiceAgentEnabled: newUserForm.isVoiceAgentEnabled ?? editingUser?.isVoiceAgentEnabled ?? false
        };

        // 1. Save locally (Always for persistence/speed)
        localDbService.saveUser(userData);
        ...
        try {
            ...
            if (!res.ok) {
                ...
                console.error(`Backend ${method} failed:`, err);
            } else {
                console.log(`✅ User ${isEdit ? 'updated' : 'created'} in backend`);
            }
        } catch (err) {
    ...
    const handleToggleVoiceAgent = (u: User, val: boolean) => {
        const updated = { ...u, isVoiceAgentEnabled: val };
        if (localDbService.saveUser(updated)) {
            loadUsersFromDb();
            playSound('success');
        }
    };
    ...
    const grossSales = filteredTickets.reduce((acc, t) => acc + (!t.isRelocation ? t.grandTotal : 0), 0);
    const droppedAmount = filteredTickets.reduce((acc, t) => acc + (t.isRelocation ? t.grandTotal : 0), 0);

    // NEW: Calculate Dropped Profit based on saved commissionAmounts
    const droppedProfit = filteredTickets.reduce((acc, t) => {
        if (t.isRelocation) {
            return acc + t.plays.reduce((playAcc, p) => playAcc + (p.commissionAmount || 0), 0);
        }
        return acc;
    }, 0);

    const netBalance = grossSales - droppedAmount;

    // Keep totalSales as grossSales for other dependencies that might need it
    const totalSales = grossSales;
    const totalPlays = filteredTickets.reduce((acc, t) => acc + t.plays.length, 0);

    // Net profit now subtracts the dropped amount as well, but ADDS the dropped profit
    const netProfit = netBalance - auditStats.totalPayout + droppedProfit;

    // --- NEW CANCEL TICKET LOGIC ---
    const handleCancelTicket = async (ticket: TicketData, e: React.MouseEvent) => {
        e.stopPropagation(); // Prevent opening ticket modal
        const pin = prompt('Enter Admin PIN to cancel this ticket:');
        if (pin !== '198312') {
            alert('Invalid PIN');
            return;
        }

        if (window.confirm(`Are you sure you want to CANCEL ticket ${ticket.ticketNumber}? This will revert sales and delete the ticket.`)) {
            try {
                const adminName = users.find(u => u.role === 'admin')?.name || 'Admin';

                // 1. Delete from local DB
                await localDbService.deleteTicket(ticket.ticketNumber);

                // 2. Add void entry to Ledger
                await localDbService.addToLedger({
                    action: 'PAYOUT', // Types.ts compatible refund
                    userId: ticket.userId,
                    amount: ticket.grandTotal, // Positive amount to refund/void the Wager
                    details: `Voided Ticket #${ticket.ticketNumber} by ${adminName}`
                });
                ...
                await localDbService.addAuditLog({
                    id: Date.now().toString(),
                    timestamp: new Date().toISOString(),
                    action: 'DELETE', // Types.ts compatible
                    details: `Cancelled Ticket ${ticket.ticketNumber} (Amount: $${ticket.grandTotal.toFixed(2)})`,
                    user: adminName,
                    targetId: ticket.ticketNumber
                });

                // Optimistically update UI
                setTickets(prev => prev.filter(t => t.ticketNumber !== ticket.ticketNumber));
                setFilteredTickets(prev => prev.filter(t => t.ticketNumber !== ticket.ticketNumber));

                playSound('delete');
                alert(`Ticket ${ticket.ticketNumber} cancelled successfully.`);

                // Delete from Backend as well (fire and forget)
                fetch(`/api/tickets/${ticket.ticketNumber}`, {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' }
                }).catch(e => console.error("Failed to delete ticket from backend", e));
