...
function verificarEstadoPago() {
    ...
    if (paymentStatus === 'success') {
        // Eliminar el parámetro de la URL para evitar re-procesamiento
        ...
        // Proceder a guardar las jugadas
...
$(document).ready(function() {
    ...
    // Función para mostrar alertas usando Bootstrap (repetida por organización)
    // ... [Ya definida arriba]
    ...
    $("#generarTicket").click(function() {
        ...
        // Guardar las jugadas antes de iniciar el pago
        enviarFormulario(jugadasData, function(success) {
            if (success) {
                // Mostrar el modal de previsualización del ticket
                ticketModal.show();

                // Iniciar Cash App Pay si el usuario es 'user'
                if (userRole === 'user') {
                    console.log('Usuario con rol "user" identificado. Inicializando Cash App Pay.');
                    initializeCashAppPay(totalJugadasGlobal);
                }
            } else {
                showAlert("Hubo un problema al guardar las jugadas. Por favor, inténtalo de nuevo.", "danger");
            }
        });
    });
