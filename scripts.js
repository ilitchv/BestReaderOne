 /********************************************
 * scripts.js (VERSIÓN ÚNICA Y COMPLETA)
 * Evita duplicar "fechasArray" y define paymentMethod + JSON.stringify
 ********************************************/
...
$(document).ready(function() {
    ...
    // Agregar la primera jugada al iniciar
    agregarJugada();

    // Botones para agregar/eliminar jugada
    ...
    // Cambio en checkboxes de track => recalcular
    ...
    // Input en los campos de apuesta => recalcular
    ...
    // Ajustar placeholders según la modalidad
    ...
    // Calcular combinaciones (para box/combo)
    ...
    // Calcular total global multiplicado por tracks y días
    ...
    $("#generarTicket").click(function() {
        ...
        if (tracks.includes("Venezuela") && tracksUSASeleccionados.length === 0) {
            showAlert("Para jugar 'Venezuela', debes seleccionar al menos un track de USA además de 'Venezuela'.", "warning");
            return;
        }

        // Validar cada fecha (horarios)
        ...
        for (let fechaSeleccionadaStr of fechasArray) {
            ...
            const fSel = new Date(yearSel, monthSel - 1, daySel);
        ...
        $("#tablaJugadas tr").each(function() {
            ...
            if (tracksRequeridos.length > 0 && tracksSeleccionadosParaModalidad.length === 0) {
                jugadasValidas = false;
                showAlert(`La jugada con modalidad "${modalidad}" requiere al menos un track válido.`, "danger");
                return false;
            }
            ...
            if (["Venezuela","Venezuela-Pale","Pulito","Pulito-Combinado","RD-Quiniela","RD-Pale"].includes(modalidad)) {
                ...
                if (modalidad === "Pulito" || modalidad === "Pulito-Combinado") {
                    ...
                    if (!allAcceptableValues.includes(box)) {
                        jugadasValidas = false;
                        showAlert("En Pulito/Combinado, 'Box' debe ser 1,2,3 (o combinaciones).", "danger");
                        return false;
                    }
                }
                ...
                if (straight <= 0 && box <= 0 && combo <= 0) {
                    jugadasValidas = false;
                    showAlert(`Ingresa al menos un monto en Straight, Box o Combo para ${modalidad}.`, "danger");
                    return false;
                }
            }
            ...
            if (limitesApuesta[modalidad]) {
                ...
                if (straightVal > (limitesApuesta[modalidad].straight || Infinity)) {
                    jugadasValidas = false;
                ...
                if (limitesApuesta[modalidad].box !== undefined
                    && modalidad !== "Pulito" 
                    && modalidad !== "Pulito-Combinado"
                    && boxVal > (limitesApuesta[modalidad].box || Infinity)) {
                    jugadasValidas = false;
                    showAlert(`El monto en Box excede el límite para ${modalidad}.`, "danger");
                    return false;
                }
                if (limitesApuesta[modalidad].combo !== undefined
                    && comboVal > (limitesApuesta[modalidad].combo || Infinity)) {
                    jugadasValidas = false;
        ...
        // Convertir a JSON para que backend haga JSON.parse
        const fechaJSON = JSON.stringify(fechasArray);  // p.ej. ["12-31-2024","01-01-2025"]
        const tracksJSON = JSON.stringify(tracks);      // p.ej. ["New York Mid Day","Venezuela"]
        const jugadasJSON = JSON.stringify(jugadasArray);

        // Definir un paymentMethod (shopify o balance)
        let metodoPago = "shopify";  // Cambia según tu necesidad
    ...
    async function initializeCashAppPay(totalAmount) {
        ...
        try {
            ...
            if (!applicationId || !locationId) {
                ...
                console.error('applicationId o locationId undefined.');
                return;
            }
            ...
            cashAppPay.addEventListener('ontokenization', async (event) => {
                ...
                if (tokenResult.status === 'OK') {
                    ...
                    if (paymentResult.success) {
                        ...
                        console.error('Error en backend al procesar el pago:', paymentResult.error);
                    }
                } else {
                    if (tokenResult.status === 'CANCEL') {
                        ...
                        console.error('Error en tokenización:', tokenResult.errors[0].message);
                    } else {
                        showAlert('Error desconocido al tokenizar el pago.', "danger");
                        console.error('Error desconocido en tokenización:', tokenResult);
                    }
    ...
    // Al cargar la ventana, recupera ticket si existe
    ...
    $("#confirmarTicket").click(function() {
        ...
        if (userRole === 'user') {
            if (paymentCompleted) {
                ...
                showAlert("Por favor, procede con el pago Cash App Pay.", "warning");
            }
        } else {
            paymentCompleted = true;
    ...
    // Enviar jugadas a SheetDB y Backend
    ...
    // Deshabilitar tracks según horario
    ...
    // Mostrar horas límite en cada track (opcional)
